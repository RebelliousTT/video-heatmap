...
class CotLayer(nn.Module):
    def __init__(self, dim, num_segments=8, typee=1,kernel_size=3):
        ...
        self.embed = nn.Sequential(
            nn.Conv2d(3 * dim, dim // factor, 1, bias=False),
        ...
        self.l_s=nn.Sequential(
            AttentionModule(dim=dim, num_segments=num_segments, typee=typee),
            nn.BatchNorm2d(dim),
            nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        k_l = k_l.view(B, C, 1, N, H, W)
        x = torch.cat([x, k, k_l], dim=2)
        x_gap = x.sum(dim=2)
...
class MKAttention(nn.Module):
    def __init__(self, dim=64, dim_mid=64, num_segments=8, typee=1):
        ...
        self.spatial_temporal_unit = CotLayer(dim=dim_mid, num_segments=num_segments, typee=typee, kernel_size=3)#########
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, num_segments=8, downsample=None, groups=1, base_width=64,
        ...
        if mka:
            self.mka = MKAttention(dim=inplanes, dim_mid=inplanes//mka_squeeze, num_segments=8, typee=mka_typee)
        else:
