...
__all__ = ['mkaresnet50', 'mkaresnet18']
def pair(t):
...
class AttentionModule(nn.Module):
    def __init__(self, dim=64, num_segments=8, typee=1):
        ...
        # self.conv3 = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1)
        # self.conv6 = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv3d(dim, dim, kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0))
        self.conv3 = nn.Conv3d(dim, dim, kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0))
        # self.conv6 = nn.Conv3d(dim, dim, kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0))
    ...
    def forward(self, x):
        ...
        # attn = rearrange(attn, '(b n) c h w -> (b h w) c n', n=self.num_segments)
        attn = rearrange(attn, '(b n) c h w -> b c n h w', n=self.num_segments)
        attn = self.conv2(attn)
        attn = self.conv3(attn)
        # attn = self.conv6(attn)
        # attn = rearrange(attn, '(b h w) c n -> (b n) c h w', h=h, w=w)
        attn = rearrange(attn, 'b c n h w -> (b n) c h w', h=h, w=w)

        return u * attn

class MKAttention(nn.Module):
    def __init__(self, dim=64, dim_mid=64, num_segments=8, typee=1, shift=False):
        super().__init__()

        self.proj_1 = nn.Conv2d(dim, dim_mid, 1)
        self.activation = nn.ReLU(inplace=True)
        self.spatial_gating_unit = AttentionModule(dim=dim_mid, num_segments=num_segments, typee=typee)
        self.proj_2 = nn.Conv2d(dim_mid, dim, 1)

        # self.bn1 = nn.BatchNorm2d(dim_mid)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, num_segments=8, downsample=None, groups=1, base_width=64,
        ...
        if mka:
            self.mka = MKAttention(dim=inplanes, dim_mid=inplanes//mka_squeeze, num_segments=8, typee=mka_typee, shift=False)
        else:
    ...
    def forward(self, x):
        ...
        out = self.shift(x)
        out = self.conv1(out)
        # out = rearrange(out, '(b n) c h w -> b c n h w', n=self.num_segments)
        out = self.bn1(out)
        # out = rearrange(out, 'b c n h w -> (b n) c h w', n=self.num_segments)
        out = self.relu(out)
...
class Model(nn.Module):
    ...
    def __init__(self, num_class, num_segments, dropout, block=Bottleneck, layers=[3, 4, 6, 3], zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block, 64, layers[0],
                                       shift=True, shift_grad=False, mka=True, mka_typee=1, mka_squeeze=mka_squeeze)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0],
                                       shift=True, shift_grad=False, mka=True, mka_typee=1, mka_squeeze=mka_squeeze)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1],
                                       shift=True, shift_grad=False, mka=True, mka_typee=2, mka_squeeze=mka_squeeze)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2],
                                       shift=True, shift_grad=False, mka=True, mka_typee=3, mka_squeeze=mka_squeeze)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * block.expansion, num_class)
        # if block == Bottleneck:
            # self.ATTEN1 = MKAttention(dim=64, dim_mid=64, num_segments=8, typee=1, shift=True)

        # if block == BasicBlock:
            # self.ATTEN1 = MKAttention(dim=64, dim_mid=32, num_segments=8, typee=1, shift=True)
    ...
    def get_optim_policies(self):
        ...
        for m in self.modules():
            ...
            elif isinstance(m, torch.nn.BatchNorm2d) or isinstance(m, torch.nn.BatchNorm2d):
...
def mkaresnet50(num_classes=174, num_segments=16, drop_rate=0., drop_path_rate=0., **kwargs):
...
def mkaresnet18(num_classes=174, num_segments=16, drop_rate=0.5, drop_path_rate=0., **kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    ...
    model = Model(num_classes, num_segments, drop_rate, block=BasicBlock, layers=[2, 2, 2, 2],
                  zero_init_residual=False,
                  groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=nn.BatchNorm2d, mka_squeeze=2)
    return model
...
if __name__ == "__main__":
    ...
    net = mkaresnet50(num_classes=174, num_segments=8, drop_rate=0., drop_path_rate=0.).cuda()
